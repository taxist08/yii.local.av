...
class adLDAPUsers
{
    ...
    public function create($attributes)
    {
        ...
        if (array_key_exists("password", $attributes) && (!$this->adldap->getUseSSL() &&
            !$this->adldap->getUseTLS())) {
        ...
        // Add the entry
        $result = @ldap_add($this->adldap->getLdapConnection(), "CN=" . $add["cn"][0] .
            ", " . $container . "," . $this->adldap->getBaseDn(), $add);
        if ($result != true) {
            return false;
        }
    ...
    public function info($username, $fields = null, $isGUID = false)
    {
        ...
        $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(),
            $filter, $fields);
    ...
    public function passwordExpiry($username, $isGUID = false)
    {
        ...
        $sr = ldap_read($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(),
            'objectclass=*', array('maxPwdAge'));
        if (!$sr) {
            return false;
        }
        ...
        $status['expiryformat'] = date('Y-m-d H:i:s', bcsub(bcdiv($pwdExpire, '10000000'),
            '11644473600'));
    ...
    public function all($includeDescription = false, $search = "*", $sorted = true)
    {
        ...
        $filter = "(&(objectClass=user)(samaccounttype=" . adLDAP::
            ADLDAP_NORMAL_ACCOUNT . ")(objectCategory=person)(cn=" . $search . "))";
    ...
    /**
     * Return a list of all enabled and no hidden users in AD
     * 
     * @param bool $includeDescription Return a description of the user
     * @param string $search Search parameter
     * @param bool $sorted Sort the user accounts
     * @return array
     */
    public function all_valid($includeDescription = false, $search = "*", $sorted = true)
    {
        if (!$this->adldap->getLdapBind()) {
            return false;
        }
        ...
        // Perform the search and grab all their details
        $filter = "(&(objectCategory=person)(objectClass=user)(!(useraccountcontrol:1.2.840.113556.1.4.803:=2))(!(memberOf=CN=hidden,CN=Users,DC=avmg,DC=loc)))";
        $fields = array(
            "displayName",
            "mail",
            "telephonenumber",
        ...
        $usersArray = array();
        foreach ($entries as $key => $person) {
            if (is_array($person))
                foreach ($person as $fields) {
                    if (!is_array($fields) && @$person[$fields][0])
                        $usersArray[$key][$fields] = ($person[$fields][0]);
                }
    ...
    public function usernameToGuid($username)
    {
        ...
        if (ldap_count_entries($this->adldap->getLdapConnection(), $sr) > 0) {
            ...
            $guid = @ldap_get_values_len($this->adldap->getLdapConnection(), $entry,
                'objectGUID');
     ...
     * @return array
     */
    public function find($includeDescription = false, $searchField = false, $searchFilter = false,
        $sorted = true)
    {
        if (!$this->adldap->getLdapBind()) {
            return false;
        ...
        $filter = "(&(objectClass=user)(samaccounttype=" . adLDAP::
            ADLDAP_NORMAL_ACCOUNT . ")(objectCategory=person)" . $searchParams . ")";
        $fields = array("samaccountname", "displayname");
        $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(),
            $filter, $fields);
        $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);
